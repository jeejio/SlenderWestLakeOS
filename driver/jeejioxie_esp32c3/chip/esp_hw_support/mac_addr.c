...
esp_err_t esp_read_mac(uint8_t *mac, esp_mac_type_t type)
{
    ...
    if (esp_base_mac_addr_get(efuse_mac) != ESP_OK) 
    {
        ...
        LOGI(TAG, "===[default mac addr]=====0x%x-0x%x-0x%x-0x%x-0x%x-0x%x=======", efuse_mac[0],efuse_mac[1],efuse_mac[2],efuse_mac[3],efuse_mac[4],efuse_mac[5]);
...
#ifdef CONFIG_JEEJIO_TARGET_ESP32S2
    ...
    case ESP_MAC_BT:
        ...
        memcpy(mac, efuse_mac, 6);
#if !CONFIG_JEEJIO_TARGET_ESP32H2
...
#endif //!CONFIG_JEEJIO_TARGET_ESP32H2
#else
