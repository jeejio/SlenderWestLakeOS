...
#if CONFIG_JEEJIO_TARGET_ARCH_RISCV && !CONFIG_ESP_SYSTEM_USE_EH_FRAME
...
#endif // CONFIG_JEEJIO_TARGET_ARCH_RISCV && !CONFIG_ESP_SYSTEM_USE_EH_FRAME
...
#if CONFIG_JEEJIO_TARGET_ARCH_RISCV
...
#else // CONFIG_JEEJIO_TARGET_ARCH_RISCV
...
#endif // CONFIG_JEEJIO_TARGET_ARCH_RISCV
...
//                if (task_affinity == 0) {
    ...
    for (int x = 0; x < portNUM_PROCESSORS; x++) {
        ESP_EARLY_LOGE(TAG, "CPU %d: %s", x, pcTaskGetName(xTaskGetCurrentTaskHandle()));
    }
    portEXIT_CRITICAL_ISR();
