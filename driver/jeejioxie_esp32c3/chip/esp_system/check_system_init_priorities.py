...
def main() -> None:
    try:
        jeejio_path = os.environ['JEEJIO_PATH']
    except KeyError:
        raise SystemExit('JEEJIO_PATH must be set before running this script')
    ...
    for extension in ('c', 'cpp'):
        glob_iter = glob.glob(os.path.join(jeejio_path, 'driver', 'jeejioxie_esp32c3', 'chip', '**', f'*.{extension}'), recursive=True)
    ...
    for filename in itertools.chain(*source_files_iters):
        ...
        for match in found:
            entry = StartupEntry(
                filename=os.path.relpath(filename, jeejio_path),
                func=match[0],
                affinity=match[1],
    ...
    with open(os.path.join(jeejio_path, STARTUP_ENTRIES_FILE), 'r') as startup_entries_expected_file:
