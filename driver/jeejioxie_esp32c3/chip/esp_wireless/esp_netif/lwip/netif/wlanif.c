...
#ifndef RT_DEVICE
#define RT_DEVICE(__device) ((jee_device_t)__device)
#endif
...
/**
 * 用来配置netif中与底层以太网网卡息息相关的成员变量以及调用网卡驱动的初始化函数初始化底层网卡
 * 
...
err_t wlanif_output(struct netif *netif, struct pbuf *p)
{
...
static err_t wlan_linkoutput(struct netif *netif, struct pbuf *p)
{
    jee_err_t ret = JEE_EOK;
    struct rt_wlan_device *wdev = jee_device_find(WLAN_NAME);
    if (wdev == NULL) {
        ESP_LOGE("wlan_linkoutput", "===================wlan dev is null");
        return ERR_IF;
    }

    struct wlan_trans_output_func_args woargs = {
        .wlanif = netif,
        .pbuf = p,
    };
    ret = jee_device_control(RT_DEVICE(wdev), RT_WLAN_CMD_OUTPUT, &woargs);
    return (err_t)ret;
}

static char inet_buff[18];
static const char *net_ipv4_ntoa(long ipaddr)
{
...
void wlanif_input(void *h, void *buffer, size_t len, void* l2_buff)
{
    ...
    {
        LWIP_DEBUGF(LWIP_DBG_ON, ("wlanif_input: IP input error\n"));
        pbuf_free(p);
    }
}

void wlan_input(void *h, void *buffer, size_t len, void* l2_buff)
{
    jee_err_t ret = JEE_EOK;
    struct rt_wlan_device *wdev = jee_device_find(WLAN_NAME);
    if (wdev == NULL) {
        ESP_LOGE("wlan_input", "===================wlan dev is null");
        return;
    }

    struct wlan_trans_input_func_args wiargs = {
        .wlanif = h,
        .buffer = buffer,
        .len = len,
        .l2_buff = l2_buff,
    };
    ret = jee_device_control(RT_DEVICE(wdev), RT_WLAN_CMD_INPUT, &wiargs);
    if (ret != JEE_EOK) {
        ESP_LOGE("wlan_input", "===================control RT_WLAN_CMD_INPUT failed");
    }
}
...
#endif /* LWIP_IPV6 */
    netif->linkoutput = wlan_linkoutput; //low_level_output;
