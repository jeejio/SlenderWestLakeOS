...
#include "lb1427b.h"    //厂家驱动头文件
#include "string.h"
#include "auto_init.h"
#include "driver/gpio.h"
#include "drv_sensor_lb1427b.h"
...
{
    printf("lb1427 sensor_control enter [cmd ] %d\n", cmd);

    switch (cmd)
    {
    case WATER_PUMP_SET_CMD:
    {
        lb1427SetPwmDuty(*(uint8_t *)arg);
    }
    break;
    case JEE_SENSOR_CTRL_SET_POWER:
    {
        ...
        {
            printf(" start : lb1427Init \n");
            lb1427Init();
            printf(" end   : lb1427Init \n");
        }
        break;
...
static struct jee_sensor_ops sensor_ops =
 ...
 /**
   ...
   *
   */
int sensorLb1427bInit(const char *name, struct jee_sensor_config *cfg)
{
    jee_int8_t result;
    ...
    sensor_WaterPump->info.type = JEE_SENSOR_CLASS_WAPUMP;
    sensor_WaterPump->info.vendor = JEE_SENSOR_VENDOR_UNKNOWN;
    sensor_WaterPump->info.model = "lb1427b";
...
int jeeHwSensorLb1427bInit()
{
    ...
    cfg.intf.dev_name = "pwm0";
    cfg.intf.user_data = (void *)0;
    sensorLb1427bInit("lb1427b", &cfg);

    //printf("finish to execute jee_hw_sensor_DHT20_init\n");
    return JEE_EOK;
}

INIT_DEVICE_EXPORT(jeeHwSensorLb1427bInit);
