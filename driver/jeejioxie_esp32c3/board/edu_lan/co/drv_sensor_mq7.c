...
{
    printf("mq7_init sensor_control enter [cmd ] %d\n", cmd);
...
static struct jee_sensor_ops sensor_ops =
 ...
 /**
   ...
   *
   */
int sensor_mq7_init(const char *name, struct jee_sensor_config *cfg)
{
    jee_int8_t result;
    ...
    sensor_getsture->info.type = JEE_SENSOR_CLASS_CO;
    sensor_getsture->info.vendor = JEE_SENSOR_VENDOR_UNKNOWN;
    sensor_getsture->info.model = "mq7";
...
__exit:
 ...
 /**
   ...
   *
   */
int jee_hw_sensor_mq7_init()
{
    //printf("begin to execute jee_hw_sensor_mq7_init\n");
    // i2c_master_init();

    struct jee_sensor_config cfg = {0};
    cfg.intf.dev_name = "iic0";
    cfg.intf.user_data = NULL;
    sensor_mq7_init("mq7", &cfg);

    //printf("finish to execute jee_hw_sensor_mq7_init\n");
    return JEE_EOK;
}

INIT_DEVICE_EXPORT(jee_hw_sensor_mq7_init);
