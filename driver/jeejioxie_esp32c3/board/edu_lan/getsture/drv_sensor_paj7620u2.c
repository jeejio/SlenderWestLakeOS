...
{
    printf("paj7620u2 sensor_control enter [cmd ] %d\n", cmd);
    ...
    case JEE_SENSOR_CTRL_SET_POWER:
    {
        ...
        {
            printf(" start : paj7620u2 \n");
            gesture_sensor_init();
            printf(" end   : paj7620u2 \n");
        }
        break;
...
static struct jee_sensor_ops sensor_ops =
 ...
 /**
   ...
   *
   */
int sensor_paj7620u2_init(const char *name, struct jee_sensor_config *cfg)
{
    jee_int8_t result;
    ...
    sensor_getsture->info.type = JEE_SENSOR_CLASS_GETSTURE;
    sensor_getsture->info.vendor = JEE_SENSOR_VENDOR_UNKNOWN;
    sensor_getsture->info.model = "paj7620u2";
...
__exit:
 ...
 /**
   ...
   *
   */
int jee_hw_sensor_paj7620u2_init()
{
    //printf("begin to execute jee_hw_sensor_paj7620u2_init\n");
    // i2c_master_init();

    struct jee_sensor_config cfg = {0};
    cfg.intf.dev_name = "iic0";
    cfg.intf.user_data = NULL;
    sensor_paj7620u2_init("paj7620u2", &cfg);

    //printf("finish to execute jee_hw_sensor_paj7620u2_init\n");
    return JEE_EOK;
}

INIT_DEVICE_EXPORT(jee_hw_sensor_paj7620u2_init);
