 ...
 * 2023-02-09    xuyuhu          Change the place where [cc6201stInit] is placed  
 */
...
static jee_size_t sensorFetchData(struct jee_sensor_device *sensor, void *buf, jee_size_t len)
{
    ...
    uint8_t *magdata = (uint8_t *)buf;
    *magdata = cc6201stGetStatus();
...
{
    printf("cc6201 sensorControl enter [cmd ] %d\n", cmd);
    ...
    case JEE_SENSOR_CTRL_SET_POWER:
    {
        ...
        {
            printf(" start : cc6201stInit \n");
            cc6201stInit();
            printf(" end   : cc6201stInit \n");
        }
        break;
...
static struct jee_sensor_ops sensor_ops =
 ...
 /**
   ...
   *
   */
int sensorCc6201stInit(const char *name, struct jee_sensor_config *cfg)
{
    jee_int8_t result;
    ...
    sensor_Magnetometer->info.type = JEE_SENSOR_CLASS_MAG;
    sensor_Magnetometer->info.vendor = JEE_SENSOR_VENDOR_UNKNOWN;
    sensor_Magnetometer->info.model = "cc6201st";
