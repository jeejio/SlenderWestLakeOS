 ...
 * Change Logs:
 * Date           Author         Notes
 * 2023-02-02     xuyuhu         the first version
 */

#ifndef __JEE_SENSOR_EC11_H__
#define __JEE_SENSOR_EC11_H__

#include "device.h"
#include "hal_sensor.h" //sesor IO模型驱动框架头文件
#include "ec11.h"       //厂家驱动头文件
#include "string.h"
#include "auto_init.h"

jee_size_t sensorEc11FetchData(struct jee_sensor_device *sensor, void *buf, jee_size_t len)
{
    if (buf == NULL)
        return 0;
    printf("ec11SensorFetchData\n");
    encoderGetDirectinPressedRelativePos((uint32_t *)buf);
    return (strlen(buf) - 1);
}

jee_err_t sensorEc11Control(struct jee_sensor_device *sensor, int cmd, void *arg)
{
    ...
    case JEE_SENSOR_CTRL_SET_POWER:
    {
        ...
        {
            printf(" start : encoderInit \n");
            encoderInit();
            printf(" end   : encoderInit \n");
        }
        break;
...
int sensorEc11Init(const char *name, struct jee_sensor_config *cfg)
{
    jee_int8_t result;
    ...
    sensor->info.type = JEE_SENSOR_CLASS_RE;
    sensor->info.vendor = JEE_SENSOR_VENDOR_UNKNOWN;
    sensor->info.model = "EC11";
...
#define GPIO_DEV_NAME "GPIO_encoder"
int jeeHwSensorEc11Init()
{
    //printf("begin to execute jeeHwSensorEc11Init\n");

    struct jee_sensor_config cfg = {0};
    cfg.intf.dev_name = GPIO_DEV_NAME;
    sensorEc11Init("EC11", &cfg);
    //printf("finish to execute jeeHwSensorEc11Init\n");
    return JEE_EOK;
}

INIT_DEVICE_EXPORT(jeeHwSensorEc11Init);
