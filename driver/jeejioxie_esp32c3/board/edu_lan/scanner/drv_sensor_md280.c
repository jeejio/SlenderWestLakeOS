...
#include "md280.h"    //厂家驱动头文件
#include "auto_init.h"
#include "driver/gpio.h"
#include "drv_sensor_md280.h"
...
{
    printf("md280Init sensor_control enter [cmd ] %d\n", cmd);
    ...
    case JEE_SENSOR_CTRL_SET_POWER:
    {
        ...
        {
            printf(" start : md280Init \n");
            md280Init();
            printf(" end   : md280Init \n");
        }
        break;
...
static struct jee_sensor_ops sensor_ops =
 ...
 /**
   ...
   *
   */
int sensorMd280Init(const char *name, struct jee_sensor_config *cfg)
{
    jee_int8_t result;
    ...
    sensor_WaterPump->info.type = JEE_SENSOR_CLASS_SCANNER;
    sensor_WaterPump->info.vendor = JEE_SENSOR_VENDOR_UNKNOWN;
    sensor_WaterPump->info.model = "md280";
