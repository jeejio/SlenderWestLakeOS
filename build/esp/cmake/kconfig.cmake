...
function(__kconfig_init)
    ...
    idf_build_set_property(__ROOT_KCONFIG ${jeejio_path}/build/esp/Kconfig)
    idf_build_set_property(__ROOT_SDKCONFIG_RENAME ${jeejio_path}/build/esp/sdkconfig.rename)
...
function(__kconfig_component_init component_target)
    ...
    file(GLOB sdkconfig_rename_target "${component_dir}/sdkconfig.rename.${JEEJIO_TARGET}")
...
function(__kconfig_generate_config sdkconfig sdkconfig_defaults)
    ...
    configure_file("${jeejio_path}/build/esp/kconfig_new/config.env.in" ${config_env_path})
    idf_build_set_property(CONFIG_ENV_PATH ${config_env_path})

    if(sdkconfig_defaults)
        foreach(sdkconfig_default ${sdkconfig_defaults})
            list(APPEND defaults_arg --defaults "${sdkconfig_default}")
            if(EXISTS "${sdkconfig_default}.${jeejio_target}")
                list(APPEND defaults_arg --defaults "${sdkconfig_default}.${jeejio_target}")
            endif()
        endforeach()
    endif()
    ...
    set(prepare_kconfig_files_command
        ${python} ${jeejio_path}/build/esp/kconfig_new/prepare_kconfig_files.py
        --list-separator=semicolon
        --env-file ${config_env_path})

    set(confgen_basecommand
        ${python} ${jeejio_path}/build/esp/kconfig_new/confgen.py
        --list-separator=semicolon
    ...
    set_property(DIRECTORY APPEND PROPERTY CMAKE_CONFIGURE_DEPENDS "${jeejio_path}/build/esp/kconfig_new/confgen.py")
    ...
    add_custom_target(menuconfig
        ...
        COMMAND ${prepare_kconfig_files_command}
        COMMAND ${confgen_basecommand}
        --env "JEEJIO_TARGET=${jeejio_target}"
    ...
    add_custom_target(confserver
        COMMAND ${prepare_kconfig_files_command}
        COMMAND ${PYTHON} ${JEEJIO_PATH}/build/esp/kconfig_new/confserver.py
        --env-file ${config_env_path}
        --kconfig ${JEEJIO_PATH}/Kconfig
        --sdkconfig-rename ${root_sdkconfig_rename}
        --config ${sdkconfig}
        VERBATIM
        USES_TERMINAL)
