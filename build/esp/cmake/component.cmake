...
macro(__component_set_properties)
    ...
    __component_set_property(${component_target} REQUIRED_JEEJIO_TARGETS "${__REQUIRED_JEEJIO_TARGETS}")
...
function(__component_get_requirements)
    ...
    execute_process(COMMAND "${CMAKE_COMMAND}"
        ...
        -P "${jeejio_path}/build/esp/cmake/scripts/component_get_requirements.cmake"
...
macro(__component_check_target)
    if(__REQUIRED_JEEJIO_TARGETS)
        idf_build_get_property(jeejio_target JEEJIO_TARGET)
        if(NOT jeejio_target IN_LIST __REQUIRED_JEEJIO_TARGETS)
            message(FATAL_ERROR "Component ${COMPONENT_NAME} only supports targets: ${__REQUIRED_JEEJIO_TARGETS}")
        endif()
    endif()
endmacro()
...
# @param[in, optional] REQUIRED_JEEJIO_TARGETS (multivalue) the list of IDF build targets that the component only supports
...
function(idf_component_register)
    ...
    set(multi_value SRCS SRC_DIRS EXCLUDE_SRCS
                    ...
                    PRIV_REQUIRES REQUIRED_JEEJIO_TARGETS EMBED_FILES EMBED_TXTFILES)
...
function(idf_component_mock)
    ...
    set(CMOCK_DIR "${JEEJIO_PATH}/driver/jeejioxie_esp32c3/chip/cmock/CMock")
    ...
    add_custom_command(
        ...
        DEPENDS ruby_found
        COMMAND ${CMAKE_COMMAND} -E env "UNITY_DIR=${JEEJIO_PATH}/components/unity/unity"
            ruby
...
# target_link_libraries(${COMPONENT_LIB} PRIVATE jeejio::dependency_name), only if 'dependency_name'
...
# require_jeejio_targets
#
# Compatibility function for requiring IDF build targets for 3.xx style components.
function(require_jeejio_targets)
    set(__REQUIRED_JEEJIO_TARGETS "${ARGN}")
    __component_check_target()
endfunction()
...
# Jeejio auto init list initialization
#
#
function(__jee_auto_init_list_init)
    set_property(GLOBAL PROPERTY LHS_AUTO_INIT_LIST "")
endfunction()

# Jeejio auto init list register
#
# Wrapper around LHS_AUTO_INIT_LIST that passes the component library name of auto init.
function(jee_auto_init_list_register)
    # remove last end of "/"
    string(REGEX REPLACE "/$" "" CURRENT_FOLDER_ABSOLUTE ${CMAKE_CURRENT_SOURCE_DIR})

    # get current relative dir name and set target name
    string(REGEX REPLACE ".*/(.*)" "\\1" CURRENT_FOLDER ${CURRENT_FOLDER_ABSOLUTE})
    # set (COMP_NAME ${CURRENT_FOLDER})

    message(STATUS "register \"${CURRENT_FOLDER}\" to auto init list.")
    set_property(GLOBAL APPEND PROPERTY LHS_AUTO_INIT_LIST "jeejio::${CURRENT_FOLDER}")
endfunction()