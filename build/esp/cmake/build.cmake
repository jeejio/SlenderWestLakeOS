...
#
# Initialize the build. This gets called upon inclusion of env_init.cmake to set internal
# properties used for the processing phase of the build.
#
function(__build_init jeejio_path)

    set(target ${JEEJIO_TARGET})
    ...
    elseif(CMAKE_HOST_SYSTEM_NAME MATCHES "Windows")
	    ...
	    message(FATAL_ERROR "unknow platform, python environment not set.")
    endif()
    file(TO_CMAKE_PATH ${PYTHON} PYTHON)
    idf_build_set_property(PYTHON ${PYTHON})

    idf_build_set_property(JEEJIO_PATH ${jeejio_path})

    idf_build_set_property(__PREFIX jeejio)
    ...
    set(jeejio_esp32c3_path ${jeejio_path}/driver/jeejioxie_esp32c3)
    file(GLOB component_dirs ${jeejio_esp32c3_path}/chip/*
                             ${jeejio_esp32c3_path}/chip/esp_common/*
                             ${jeejio_esp32c3_path}/chip/esp_wireless/*
                             ${jeejio_esp32c3_path}/chip/storage/*
                             ${jeejio_esp32c3_path}/board/*
                             ${jeejio_path}/kernel/arch/*
                             ${jeejio_path}/kernel/lib/*)
    LIST(APPEND component_dirs ${jeejio_path}/kernel)
    LIST(APPEND component_dirs ${jeejio_path}/hal)
    LIST(APPEND component_dirs ${jeejio_path}/hal/sensor)
    LIST(APPEND component_dirs ${jeejio_path}/driver/jeejioxie_esp32c3/chip/driver/gpio)
    LIST(APPEND component_dirs ${jeejio_path}/driver/jeejioxie_esp32c3/chip/driver/uart)
    LIST(APPEND component_dirs ${jeejio_path}/driver/jeejioxie_esp32c3/chip/driver/i2c)
    LIST(APPEND component_dirs ${jeejio_path}/driver/jeejioxie_esp32c3/chip/driver/pwm)
    LIST(APPEND component_dirs ${jeejio_path}/driver/jeejioxie_esp32c3/chip/driver/adc)
    LIST(APPEND component_dirs ${jeejio_path}/driver/jeejioxie_esp32c3/chip/driver/cputime)
    LIST(APPEND component_dirs ${jeejio_path}/driver/jeejioxie_esp32c3/chip/esp_wireless/esp_wifi)
    LIST(APPEND component_dirs ${jeejio_path}/driver/jeejioxie_esp32c3/chip/driver/spi)
    LIST(APPEND component_dirs ${jeejio_path}/driver/jeejioxie_esp32c3/chip/driver/rtc)
    ...
    set(requires_common newlib kernel esp_hw_support heap log soc esp_hal esp_errno esp_system)
    idf_build_set_property(__COMPONENT_REQUIRES_COMMON "${requires_common}")

    __kconfig_init()
    __jee_auto_init_list_init()
endfunction()
...
function(__build_expand_requirements component_target)
    ...
    #message(STATUS "reqs = ${reqs}")
    #message(STATUS "priv_reqs = ${priv_reqs}")
    if(reqs)
...
macro(idf_build_process target)
    ...
    idf_build_set_property(JEEJIO_TARGET ${target})

    if("${target}" STREQUAL "esp32" OR "${target}" STREQUAL "esp32s2" OR "${target}" STREQUAL "esp32s3")
        idf_build_set_property(JEEJIO_TARGET_ARCH "xtensa")
    elseif("${target}" STREQUAL "linux")
        # No arch specified for linux host builds at the moment
        idf_build_set_property(JEEJIO_TARGET_ARCH "")
    else()
        idf_build_set_property(JEEJIO_TARGET_ARCH "riscv")
    endif()
    ...
    idf_build_get_property(jeejio_path JEEJIO_PATH)
    add_subdirectory(${jeejio_path}/driver/jeejioxie_esp32c3 ${build_dir}/jeejio)
