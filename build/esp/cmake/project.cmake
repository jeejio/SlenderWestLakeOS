...
# call.
include(${CMAKE_CURRENT_LIST_DIR}/env_init.cmake)
...
function(paths_with_spaces_to_list variable_name)
    if("${${variable_name}}" MATCHES "[ \t]")
        ...
        execute_process(
            COMMAND ${python}
                "${jeejio_path}/build/esp/split_paths_by_spaces.py"
...
#
# Output the built components to the user. Generates files for invoking esp_monitor.py
...
function(__project_info test_components)
    ...
    configure_file("${jeejio_path}/build/esp/cmake/project_description.json.in"
...
function(__project_init components_var test_components_var)
    ...
    if(BOOTLOADER_BUILD)
        __project_component_dir("${CMAKE_CURRENT_LIST_DIR}/bootloader_start")
        __project_component_dir("${CMAKE_CURRENT_LIST_DIR}/micro-ecc")
    else()
        __project_component_dir("${CMAKE_CURRENT_LIST_DIR}/application")
        __project_component_dir("${CMAKE_CURRENT_LIST_DIR}/framework")
        __project_component_dir("${CMAKE_CURRENT_LIST_DIR}/tal")
        __project_component_dir("${CMAKE_CURRENT_LIST_DIR}/external")
    endif()
...
macro(project project_name)
    ...
    if(SDKCONFIG)
        ...
        set(sdkconfig "${CMAKE_CURRENT_LIST_DIR}/build/esp/sdkconfig")
    endif()
    ...
    message(STATUS "Building for target ${JEEJIO_TARGET}")

    idf_build_process(${JEEJIO_TARGET}
    ...
    if(jeejio::application IN_LIST build_components)
        __component_get_target(main_target jeejio::application)
        ...
        if((NOT reqs OR reqs STREQUAL managed_reqs) AND (NOT priv_reqs OR priv_reqs STREQUAL managed_priv_reqs))
            ...
            list(REMOVE_ITEM build_components jeejio::application)
    ...
    # executable. This is also used by calc_size.py to detect the target
    set(project_elf_src ${CMAKE_BINARY_DIR}/project_elf_src_${JEEJIO_TARGET}.c)
    ...
    foreach(build_component ${build_components})
        ...
        if(whole_archive)
            ...
            #message(STATUS "current build_component is ${build_component}")
            get_property(lhs_list GLOBAL PROPERTY LHS_AUTO_INIT_LIST)
            if(build_component IN_LIST lhs_list)
                #message(STATUS "===============AUTO_INIT-${build_component}===============")
                target_link_libraries(${project_elf} PRIVATE
                                       "-Wl,--whole-archive"
                                       ${build_component}
                                       "-Wl,--no-whole-archive")
    ...
    if(CMAKE_C_COMPILER_ID STREQUAL "GNU")
        ...
        set(jeejio_target "${JEEJIO_TARGET}")
        string(TOUPPER ${jeejio_target} jeejio_target)
        target_link_libraries(${project_elf} PRIVATE "-Wl,--cref" "-Wl,--defsym=JEEJIO_TARGET_${jeejio_target}=0"
        "-Wl,--Map=\"${mapfile}\"")
        unset(jeejio_target)
    endif()
    ...
    set(idf_size ${python} ${jeejio_path}/build/esp/calc_size.py)
    if(DEFINED OUTPUT_JSON AND OUTPUT_JSON)
        list(APPEND idf_size "--json")
    endif()

    # Add size targets, depend on map file, run calc_size.py
