...
from monitor_base.ansi_color_converter import get_converter
from monitor_base.argument_parser import get_parser
from monitor_base.console_parser import ConsoleParser
from monitor_base.console_reader import ConsoleReader
from monitor_base.constants import (CTRL_C, CTRL_H, DEFAULT_PRINT_FILTER, DEFAULT_TOOLCHAIN_PREFIX,
                                        ...
                                        TAG_SERIAL_FLUSH)
from monitor_base.coredump import COREDUMP_DECODE_INFO, CoreDump
from monitor_base.exceptions import SerialStopException
from monitor_base.gdbhelper import GDBHelper
from monitor_base.line_matcher import LineMatcher
from monitor_base.logger import Logger
from monitor_base.output_helpers import normal_print, yellow_print
from monitor_base.serial_handler import SerialHandler, SerialHandlerNoElf, run_make
from monitor_base.serial_reader import LinuxReader, SerialReader
from monitor_base.web_socket_client import WebSocketClient
...
def main() -> None:
    ...
    elf_file = os.path.join(os.environ['target_dir'], 'SlenderWestLakeOS.elf')
    ...
    try:
        ...
        if args.target == 'linux':
            ...
            cls = LinuxMonitor
            yellow_print('--- esp_monitor on linux ---')
        else:
            serial_instance = serial.serial_for_url(port, 115200, do_not_open=True)
            serial_instance.dtr = False
            serial_instance.rts = False

            # Pass the actual used port to callee of esp_monitor (e.g. idf.py/cmake) through `ESPPORT` environment
            # variable.
            ...
            cls = SerialMonitor
            yellow_print('')
            yellow_print('--- esp_monitor on {p.name} {p.baudrate} ---'.format(p=serial_instance))
