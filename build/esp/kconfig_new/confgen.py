...
def write_min_config(deprecated_options, config, filename):
    target_symbol = config.syms['JEEJIO_TARGET']
    # 'esp32` is harcoded here because the default value of JEEJIO_TARGET is set on the first run from the environment
...
def write_docs(deprecated_options, config, filename):
    try:
        target = os.environ['JEEJIO_TARGET']
    except KeyError:
        print('JEEJIO_TARGET environment variable must be defined!')
        sys.exit(1)
