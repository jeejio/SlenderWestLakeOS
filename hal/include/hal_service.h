 ...
 */
jee_inline void rt_list_init(jee_list_t *l)
{
    l->next = l->prev = l;
}
 ...
 */
jee_inline void rt_list_insert_after(jee_list_t *l, jee_list_t *n)
{
    l->next->prev = n;
    n->next = l->next;
 ...
 * @param l list to insert it
 */
jee_inline void rt_list_insert_before(jee_list_t *l, jee_list_t *n)
{
    l->prev->next = n;
    n->prev = l->prev;
 ...
 */
jee_inline void rt_list_remove(jee_list_t *n)
{
    n->next->prev = n->prev;
    n->prev->next = n->next;
 ...
 */
jee_inline int rt_list_isempty(const jee_list_t *l)
{
    return l->next == l;
}
 ...
 */
jee_inline unsigned int rt_list_len(const jee_list_t *l)
{
    unsigned int len = 0;
...
#define RT_SLIST_OBJECT_INIT(object) { JEE_NULL }
 ...
 */
jee_inline void rt_slist_init(jee_slist_t *l)
{
    l->next = JEE_NULL;
}

jee_inline void rt_slist_append(jee_slist_t *l, jee_slist_t *n)
{
...
jee_inline void rt_slist_insert(jee_slist_t *l, jee_slist_t *n)
{
    n->next = l->next;
    l->next = n;
}

jee_inline unsigned int rt_slist_len(const jee_slist_t *l)
{
    unsigned int len = 0;
...
jee_inline jee_slist_t *rt_slist_remove(jee_slist_t *l, jee_slist_t *n)
{
    /* remove slist head */
...
jee_inline jee_slist_t *rt_slist_first(jee_slist_t *l)
{
    return l->next;
}

jee_inline jee_slist_t *rt_slist_tail(jee_slist_t *l)
{
...
jee_inline jee_slist_t *rt_slist_next(jee_slist_t *n)
{
    return n->next;
}

jee_inline int rt_slist_isempty(jee_slist_t *l)
{
