...
jee_err_t rt_wlan_set_mode(const char *dev_name, rt_wlan_mode_t mode);
rt_wlan_mode_t rt_wlan_get_mode(const char *dev_name);

/*
 * wifi station mode interface
 */
jee_err_t rt_wlan_connect(const char *ssid, const char *password);
jee_err_t rt_wlan_connect_adv(struct rt_wlan_info *info, const char *password);
jee_err_t rt_wlan_disconnect(void);
jee_bool_t rt_wlan_is_connected(void);
jee_bool_t rt_wlan_is_ready(void);
jee_err_t rt_wlan_set_mac(jee_uint8_t *mac);
jee_err_t rt_wlan_get_mac(jee_uint8_t *mac);
jee_err_t rt_wlan_get_info(struct rt_wlan_info *info);
int rt_wlan_get_rssi(void);

/*
 * wifi ap mode interface
 */
jee_err_t rt_wlan_start_ap(const char *ssid, const char *password);
jee_err_t rt_wlan_start_ap_adv(struct rt_wlan_info *info, const char *password);
jee_bool_t rt_wlan_ap_is_active(void);
jee_err_t rt_wlan_ap_stop(void);
jee_err_t rt_wlan_ap_get_info(struct rt_wlan_info *info);
int rt_wlan_ap_get_sta_num(void);
int rt_wlan_ap_get_sta_info(struct rt_wlan_info *info, int num);
jee_err_t rt_wlan_ap_deauth_sta(jee_uint8_t *mac);
jee_err_t rt_wlan_ap_set_country(rt_country_code_t country_code);
rt_country_code_t rt_wlan_ap_get_country(void);

/*
 * wifi scan interface
 */
jee_err_t rt_wlan_scan(void);
struct rt_wlan_scan_result *rt_wlan_scan_sync(void);
jee_err_t rt_wlan_scan_with_info(struct rt_wlan_info *info);


/*
 * wifi auto connect interface
 */
void rt_wlan_config_autoreconnect(jee_bool_t enable);
jee_bool_t rt_wlan_get_autoreconnect_mode(void);

/*
 * wifi power management interface
 */
jee_err_t rt_wlan_set_powersave(int level);
int rt_wlan_get_powersave(void);

/*
 * wifi event management interface
 */
jee_err_t rt_wlan_register_event_handler(rt_wlan_event_t event, rt_wlan_event_handler handler, void *parameter);
jee_err_t rt_wlan_unregister_event_handler(rt_wlan_event_t event);
...
void rt_wlan_g_mode_set_sta(void);
void rt_wlan_g_mode_set_ap(void);
int rt_wlan_g_mode_get(void);
