...
{
    jee_uint8_t len;
    jee_uint8_t val[RT_WLAN_SSID_MAX_LENGTH + 1];
};
...
{
    jee_uint8_t len;
    jee_uint8_t val[RT_WLAN_PASSWORD_MAX_LENGTH + 1];
};
...
struct rt_wlan_info
{
    ...
    rt_wlan_ssid_t ssid;
    /* hwaddr */
    jee_uint8_t bssid[RT_WLAN_BSSID_MAX_LENGTH];
    jee_uint8_t hidden;
};
...
{
    jee_uint16_t offset;     /* Offset in bytes to start filtering (referenced to the start of the ethernet packet) */
    jee_uint16_t mask_size;  /* Size of the mask in bytes */
    jee_uint8_t *mask;       /* Pattern mask bytes to be ANDed with the pattern eg. "\xff00" (must be in network byte order) */
    jee_uint8_t *pattern;    /* Pattern bytes used to filter eg. "\x0800"  (must be in network byte order) */
};
...
struct wlan_trans_input_func_args {
    void *wlanif;
    void *buffer;
    size_t len;
    void* l2_buff;
...
struct wlan_trans_output_func_args {
    void *wlanif;
    void *pbuf;
};
...
struct rt_scan_info
{
    rt_wlan_ssid_t ssid;
    jee_uint8_t bssid[6];
    jee_int16_t channel_min;
    jee_int16_t channel_max;
    jee_bool_t passive;
};

struct rt_wlan_dev_ops
{
    jee_err_t (*wlan_init)(struct rt_wlan_device *wlan);
    jee_err_t (*wlan_mode)(struct rt_wlan_device *wlan, rt_wlan_mode_t mode);
    jee_err_t (*wlan_scan)(struct rt_wlan_device *wlan, struct rt_scan_info *scan_info);
    jee_err_t (*wlan_join)(struct rt_wlan_device *wlan, struct rt_sta_info *sta_info);
    jee_err_t (*wlan_softap)(struct rt_wlan_device *wlan, struct rt_ap_info *ap_info);
    jee_err_t (*wlan_disconnect)(struct rt_wlan_device *wlan);
    jee_err_t (*wlan_ap_stop)(struct rt_wlan_device *wlan);
    jee_err_t (*wlan_ap_deauth)(struct rt_wlan_device *wlan, jee_uint8_t mac[]);
    jee_err_t (*wlan_scan_stop)(struct rt_wlan_device *wlan);
    int (*wlan_get_rssi)(struct rt_wlan_device *wlan);
    jee_err_t (*wlan_set_powersave)(struct rt_wlan_device *wlan, int level);
    int (*wlan_get_powersave)(struct rt_wlan_device *wlan);
    jee_err_t (*wlan_cfg_promisc)(struct rt_wlan_device *wlan, jee_bool_t start);
    jee_err_t (*wlan_cfg_filter)(struct rt_wlan_device *wlan, struct rt_wlan_filter *filter);
    jee_err_t (*wlan_cfg_mgnt_filter)(struct rt_wlan_device *wlan, jee_bool_t start);
    jee_err_t (*wlan_set_channel)(struct rt_wlan_device *wlan, int channel);
    int (*wlan_get_channel)(struct rt_wlan_device *wlan);
    jee_err_t (*wlan_set_country)(struct rt_wlan_device *wlan, rt_country_code_t country_code);
    rt_country_code_t (*wlan_get_country)(struct rt_wlan_device *wlan);
    jee_err_t (*wlan_set_mac)(struct rt_wlan_device *wlan, jee_uint8_t mac[]);
    jee_err_t (*wlan_get_mac)(struct rt_wlan_device *wlan, jee_uint8_t mac[]);
    ...
    jee_err_t (*wlan_fast_connect)(void *data,jee_int32_t len);
    jee_err_t (*wlan_drv_input)(struct rt_wlan_device *wlan, void*args);
    jee_err_t (*wlan_drv_output)(struct rt_wlan_device *wlan, void* args);
};

/*
 * wlan device init
 */
jee_err_t rt_wlan_dev_init(struct rt_wlan_device *device, rt_wlan_mode_t mode);

/*
 * wlan device station interface
 */
jee_err_t rt_wlan_dev_connect(struct rt_wlan_device *device, struct rt_wlan_info *info, const char *password, int password_len);
jee_err_t rt_wlan_dev_fast_connect(struct rt_wlan_device *device, struct rt_wlan_info *info, const char *password, int password_len);
jee_err_t rt_wlan_dev_disconnect(struct rt_wlan_device *device);
int rt_wlan_dev_get_rssi(struct rt_wlan_device *device);

/*
 * wlan device ap interface
 */
jee_err_t rt_wlan_dev_ap_start(struct rt_wlan_device *device, struct rt_wlan_info *info, const char *password, int password_len);
jee_err_t rt_wlan_dev_ap_stop(struct rt_wlan_device *device);
jee_err_t rt_wlan_dev_ap_deauth(struct rt_wlan_device *device, jee_uint8_t mac[6]);

/*
 * wlan device scan interface
 */
jee_err_t rt_wlan_dev_scan(struct rt_wlan_device *device, struct rt_wlan_info *info);
jee_err_t rt_wlan_dev_scan_stop(struct rt_wlan_device *device);

/*
 * wlan device mac interface
 */
jee_err_t rt_wlan_dev_get_mac(struct rt_wlan_device *device, jee_uint8_t mac[6]);
jee_err_t rt_wlan_dev_set_mac(struct rt_wlan_device *device, jee_uint8_t mac[6]);

/*
 * wlan device powersave interface
 */
jee_err_t rt_wlan_dev_set_powersave(struct rt_wlan_device *device, int level);
int rt_wlan_dev_get_powersave(struct rt_wlan_device *device);

/*
 * wlan device event interface
 */
jee_err_t rt_wlan_dev_register_event_handler(struct rt_wlan_device *device, rt_wlan_dev_event_t event, rt_wlan_dev_event_handler handler, void *parameter);
jee_err_t rt_wlan_dev_unregister_event_handler(struct rt_wlan_device *device, rt_wlan_dev_event_t event, rt_wlan_dev_event_handler handler);
 ...
 */
jee_err_t rt_wlan_dev_enter_promisc(struct rt_wlan_device *device);
jee_err_t rt_wlan_dev_exit_promisc(struct rt_wlan_device *device);
jee_err_t rt_wlan_dev_set_promisc_callback(struct rt_wlan_device *device, rt_wlan_pormisc_callback_t callback);
void rt_wlan_dev_promisc_handler(struct rt_wlan_device *device, void *data, int len);

/*
 * wlan device filter interface
 */
jee_err_t rt_wlan_dev_cfg_filter(struct rt_wlan_device *device, struct rt_wlan_filter *filter);

/*
 * wlan device channel interface
 */
jee_err_t rt_wlan_dev_set_channel(struct rt_wlan_device *device, int channel);
int rt_wlan_dev_get_channel(struct rt_wlan_device *device);

/*
 * wlan device country interface
 */
jee_err_t rt_wlan_dev_set_country(struct rt_wlan_device *device, rt_country_code_t country_code);
rt_country_code_t rt_wlan_dev_get_country(struct rt_wlan_device *device);

/*
 * wlan device datat transfer interface
 */
jee_err_t rt_wlan_dev_report_data(struct rt_wlan_device *device, void *buff, int len);
// void rt_wlan_dev_data_ready(struct rt_wlan_device *device, int len);

/*
 * wlan device register interface
 */
jee_err_t rt_wlan_dev_register(struct rt_wlan_device *wlan, const char *name,
    const struct rt_wlan_dev_ops *ops, jee_uint32_t flag, void *user_data);
