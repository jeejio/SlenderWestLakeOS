                                     ...
                                     const char *url, const char **request)
{
    ...
    *request = path_ptr ? path_ptr : "/";
    ...
    printf("host address: %s , port: %s", session->host, port_str);
...
static int webclient_open_tls(struct webclient_session *session, const char *URI)
{
    ...
    {
        printf("no memory for tls_session buffer!");
        return -WEBCLIENT_ERROR;
    }

    if((tls_ret = mbedtls_client_init(session->tls_session, (void *)pers, strlen(pers))) < 0)
    {
        printf("initialize https client failed return: -0x%x.", -tls_ret);
        return -WEBCLIENT_ERROR;
    }
...
static int webclient_connect(struct webclient_session *session, const char *URI)
{
    ...
    if (strncmp(URI, "https://", 8) == 0)
    {
        ...
        {
            printf("connect failed, https client open URI(%s) failed!", URI);
            return -WEBCLIENT_ERROR;
        }
        session->is_tls = RT_TRUE;
#else
    ...
    if (session->is_tls == FALSE && res == RT_NULL)
    {
        rc = -WEBCLIENT_ERROR;
        goto __exit;
    }
    ...
    if (session->tls_session)
    {
        ...
        {
            printf("connect failed, https client context return: -0x%x", -tls_ret);
            return -WEBCLIENT_ERROR;
        }

        if ((tls_ret = mbedtls_client_connect(session->tls_session)) < 0)
        {
            printf("connect failed, https client connect return: -0x%x", -tls_ret);
...
int webclient_shard_head_function(struct webclient_session *session, const char *URI, int *length)
{
    ...
    if(resp_status >= 0)
    {
        *length = webclient_content_length_get(session);
        printf("The length[%04d] of real data of URI.", *length);
    }
...
int webclient_shard_position_function(struct webclient_session *session, const char *URI, int start, int length, int mem_size)
{
    ...
    for(start_position = end_position; total_len != end_position + 1;)
    {
        ...
        printf("Range: [%04d -> %04d]", start_position, end_position);
        ...
        else
        {
            if(resp_status == -WEBCLIENT_ERROR)
            {
                if(session->socket == -WEBCLIENT_ERROR)
                {
                    ...
                    {
                        printf("webclient reconnect success, retry at [%06d]", end_position);
                        end_position = start_position;
                        continue;
                    }
                    else
                    {
                        printf("webclient reconnect failed. Please retry by yourself.");
                        return -WEBCLIENT_ERROR;
                    }
                }
            }
...
{
    struct timeval timeout;
    int second = (millisecond) / portTICK_PERIOD_MS;
...
int webclient_request_header_add(char **request_header, const char *fmt, ...)
{
    ...
    {
        header = calloc(1, WEBCLIENT_HEADER_BUFSZ);
        if (header == RT_NULL)
        {
    ...
    {
        printf("not enough request header data size(%d)!", WEBCLIENT_HEADER_BUFSZ);
        return -WEBCLIENT_ERROR;
    }
